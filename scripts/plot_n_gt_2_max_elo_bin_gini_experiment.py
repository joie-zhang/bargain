#!/usr/bin/env python3
"""Corrected-Gini version of the max-Elo-bin heterogeneity experiment."""
...
def gini_shifted(values: pd.Series | np.ndarray) -> tuple[float, float, bool]:
    ...
    correction = float(arr.size / (arr.size - 1))
    corrected_gini = min(raw_gini * correction, 1.0)
    return raw_gini, corrected_gini, shifted
...
def load_data() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for run_key, group in agents.groupby("run_key", sort=False):
        raw_gini, corrected_gini, shifted = gini_shifted(group["final_utility"])
        n_values = int(group["final_utility"].replace([np.inf, -np.inf], np.nan).dropna().count())
        gini_rows.append(
            {
                "run_key": run_key,
                ...
                "payoff_gini_correction_factor": float(n_values / (n_values - 1)) if n_values > 1 else math.nan,
            }
        )
    gini_frame = pd.DataFrame(gini_rows)
    runs = runs.merge(gini_frame, on="run_key", how="left", validate="one_to_one")
    runs["elo_std"] = np.sqrt(runs["elo_variance"].clip(lower=0))
    runs.to_csv(OUT_DIR / "heterogeneous_max_elo_bin_gini_selected_run_metrics.csv", index=False)
    return runs, bins


def fit_line(frame: pd.DataFrame) -> dict[str, float]:
    data = frame[["elo_std", "payoff_gini_corrected"]].replace([np.inf, -np.inf], np.nan).dropna()
    if len(data) < 3 or data["elo_std"].nunique() < 2 or data["payoff_gini_corrected"].nunique() < 2:
        return {
            "slope": math.nan,
            "intercept": math.nan,
    ...
    result = stats.linregress(data["elo_std"].to_numpy(float), data["payoff_gini_corrected"].to_numpy(float))
    return {
...
def regression_curve_with_ci(frame: pd.DataFrame) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray] | None:
    ...
    y = data["payoff_gini_corrected"].to_numpy(dtype=float)
    ...
    delta = tcrit * se_mean
    return xs, yhat, np.clip(yhat - delta, 0.0, 1.0), np.clip(yhat + delta, 0.0, 1.0)
        ...
        row.update(
            {
                ...
                "payoff_gini_corrected_min": float(sub["payoff_gini_corrected"].min()),
                "payoff_gini_corrected_max": float(sub["payoff_gini_corrected"].max()),
                "slope_gini_per_elo_std": fit["slope"],
                "slope_gini_per_100_elo_std": fit["slope"] * 100.0
                if math.isfinite(fit["slope"])
                else math.nan,
                "slope_stderr": fit["slope_stderr"],
                "slope_stderr_per_100_elo_std": fit["slope_stderr"] * 100.0
    ...
    if show_stats:
        if math.isfinite(fit["pearson_r"]):
            label = (
                ...
                f"slope={fit['slope'] * 100.0:+.2f}/100\n"
...
def plot_overall(runs: pd.DataFrame, bins: pd.DataFrame) -> Path:
    ...
    for ax, (_, bin_row), color in zip(axes, bins.iterrows(), BIN_COLORS, strict=True):
        ...
        draw_panel(ax, sub, point_color=color, x_lim=x_lim, title=title)
    axes[0].set_ylabel("Corrected within-run payoff Gini", fontsize=11, labelpad=7)
    fig.supxlabel("Within-roster Arena Elo standard deviation", fontsize=12, y=0.02)
    fig.suptitle("Corrected payoff Gini vs Elo std, conditional on strongest-model Elo bin", fontsize=15, y=1.02)
    fig.tight_layout(rect=(0, 0.06, 1, 0.96), w_pad=1.2)
    out_path = OUT_DIR / "heterogeneous_max_elo_bin_gini_overall_with_ci.png"
...
def plot_by_n(runs: pd.DataFrame, bins: pd.DataFrame) -> Path:
    ...
    fig.suptitle("Max-Elo-bin corrected-Gini experiment, broken down by group size", fontsize=15, y=0.995)
    fig.tight_layout(rect=(0, 0.05, 1, 0.97), w_pad=0.9, h_pad=1.1)
    out_path = OUT_DIR / "heterogeneous_max_elo_bin_gini_by_n_with_ci.png"
    fig.savefig(out_path, dpi=220, bbox_inches="tight")
    plt.close(fig)
    return out_path
    ...
    for row_idx, (_, bin_row) in enumerate(bins.iterrows()):
        for col_idx, game in enumerate(GAME_ORDER):
            ...
            if col_idx == 0:
                ax.set_ylabel(row_label(bin_row), fontsize=9.2, labelpad=8)
    fig.supxlabel("Within-roster Arena Elo standard deviation", fontsize=12, y=0.03)
    fig.suptitle("Max-Elo-bin corrected-Gini experiment, broken down by game", fontsize=15, y=0.995)
    fig.tight_layout(rect=(0, 0.05, 1, 0.97), w_pad=1.0, h_pad=1.1)
    out_path = OUT_DIR / "heterogeneous_max_elo_bin_gini_by_game_with_ci.png"
    fig.savefig(out_path, dpi=220, bbox_inches="tight")
    plt.close(fig)
    return out_path
    ...
    for row_idx, (_, bin_row) in enumerate(bins.iterrows()):
        for col_idx, competition_band in enumerate(COMPETITION_ORDER):
            ...
            if col_idx == 0:
                ax.set_ylabel(row_label(bin_row), fontsize=9.2, labelpad=8)
    fig.supxlabel("Within-roster Arena Elo standard deviation", fontsize=12, y=0.03)
    fig.suptitle("Max-Elo-bin corrected-Gini experiment, broken down by competition band", fontsize=15, y=0.995)
    fig.tight_layout(rect=(0, 0.05, 1, 0.97), w_pad=1.0, h_pad=1.1)
    out_path = OUT_DIR / "heterogeneous_max_elo_bin_gini_by_competition_with_ci.png"
    fig.savefig(out_path, dpi=220, bbox_inches="tight")
    plt.close(fig)
    return out_path
...
def main() -> None:
    ...
    pd.concat([overall_summary, by_n_summary, by_game_summary, by_competition_summary], ignore_index=True).to_csv(
        OUT_DIR / "heterogeneous_max_elo_bin_gini_fit_summary.csv",
        index=False,
    )
    overall_summary.to_csv(OUT_DIR / "heterogeneous_max_elo_bin_gini_overall_fit_summary.csv", index=False)
    by_n_summary.to_csv(OUT_DIR / "heterogeneous_max_elo_bin_gini_by_n_fit_summary.csv", index=False)
    by_game_summary.to_csv(OUT_DIR / "heterogeneous_max_elo_bin_gini_by_game_fit_summary.csv", index=False)
    by_competition_summary.to_csv(
        OUT_DIR / "heterogeneous_max_elo_bin_gini_by_competition_fit_summary.csv",
        index=False,
    )
    ...
    print("Overall max-Elo-bin corrected-Gini fit summary:")
    cols = [
        "max_elo_bin_label",
        "n_runs",
        "slope_gini_per_100_elo_std",
        "p_value",
        "pearson_r",
        "r_squared",
    ]
