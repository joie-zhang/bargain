...
# VERSION 1: WITH POSITIVE AND NEGATIVE NUMBERS
...
print("\n" + "=" * 60)
print("VERSION 3: MAX UTILITY = 100 (INTEGER VALUES)")
print("=" * 60)

# Vector pairs with maximum utility = 100
# All vectors sum to 100 and have integer values

# Pair 1: Cosine similarity = 0 (orthogonal vectors)
v1_unit = np.array([50, 50, 0, 0, 0])
v2_unit = np.array([0, 0, 33, 33, 34])

# Pair 2: Cosine similarity ≈ 0.25
v3_unit = np.array([43, 33, 17, 3, 4])
v4_unit = np.array([4, 3, 17, 33, 43])

# Pair 3: Cosine similarity ≈ 0.5
v5_unit = np.array([47, 30, 11, 8, 4])
v6_unit = np.array([11, 10, 20, 33, 26])

# Pair 4: Cosine similarity ≈ 0.75
v7_unit = np.array([44, 24, 21, 4, 7])
v8_unit = np.array([17, 27, 20, 30, 6])

# Pair 5: Cosine similarity = 1 (parallel vectors)
v9_unit = np.array([33, 27, 20, 13, 7])
v10_unit = np.array([33, 27, 20, 13, 7])

pairs_v3 = [
    (v1_unit, v2_unit, 0.00),
    (v3_unit, v4_unit, 0.25),
    (v5_unit, v6_unit, 0.50),
    (v7_unit, v8_unit, 0.75),
    (v9_unit, v10_unit, 1.00)
]

for i, (a, b, target) in enumerate(pairs_v3, 1):
    actual = cosine_similarity(a, b)
    print(f"\nPair {i} (Target: {target:.2f}):")
    print(f"  Vector A: {a}")
    print(f"  Vector B: {b}")
    print(f"  Sum A: {a.sum()}, Sum B: {b.sum()}")
    print(f"  Actual cosine similarity: {actual:.4f}")
    print(f"  Error: {abs(actual - target):.4f}")

# Export pairs_v3 for use in other modules
__all__ = ['pairs_v3']