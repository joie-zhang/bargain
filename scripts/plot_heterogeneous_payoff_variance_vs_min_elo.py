#!/usr/bin/env python3
"""Plot heterogeneous payoff variance against minimum roster Elo."""
...
def fit_line(frame: pd.DataFrame) -> dict[str, float]:
    data = frame[["min_roster_elo", "payoff_variance"]].replace([np.inf, -np.inf], np.nan).dropna()
    if len(data) < 3 or data["min_roster_elo"].nunique() < 2:
        return {
            "n_runs": int(len(data)),
            "slope": math.nan,
            "slope_per_100_elo": math.nan,
            "intercept": math.nan,
            "pearson_r": math.nan,
            "r_squared": math.nan,
            "p_value": math.nan,
    ...
    fit = stats.linregress(
        data["min_roster_elo"].to_numpy(dtype=float),
        data["payoff_variance"].to_numpy(dtype=float),
    )
    return {
        "n_runs": int(len(data)),
        "slope": float(fit.slope),
        "slope_per_100_elo": float(fit.slope * 100.0),
...
def draw_scatter_axis(
    ...
    if len(frame):
        ax.scatter(
            frame["min_roster_elo"],
            frame["payoff_variance"],
            s=point_size,
            alpha=0.38,
            linewidths=0,
            color=POINT_COLOR,
    ...
    if math.isfinite(fit["slope"]) and len(frame):
        xs = np.linspace(float(frame["min_roster_elo"].min()), float(frame["min_roster_elo"].max()), 160)
...
def plot_grid(
    ...
    for row_idx, (row_label, row_filter, row_extra) in enumerate(row_groups):
        for col_idx, (col_label, col_filter, col_extra) in enumerate(col_groups):
            ...
            draw_scatter_axis(ax, sub, x_lim, y_lim, point_size=18.0)
            if row_idx == 0:
                ax.set_title(col_label, fontsize=9, pad=5)
            if col_idx == 0:
                ax.set_ylabel(f"{row_label}\nPayoff variance" if len(row_groups) > 1 else "Payoff variance", fontsize=8.5)
            if row_idx == len(row_groups) - 1:
                ax.set_xlabel("Min roster Elo", fontsize=8.5)
            else:
...
def plot_overall(frame: pd.DataFrame, x_lim: tuple[float, float], y_lim: tuple[float, float]) -> tuple[Path, pd.DataFrame]:
    ...
    ax.set_xlabel("Minimum roster Elo", fontsize=11)
    ax.set_ylabel("Payoff variance", fontsize=11)
    ax.set_title("Heterogeneous runs: payoff variance vs minimum roster Elo", fontsize=13, pad=10)
    handles = [
        Line2D([0], [0], marker="o", color="none", markerfacecolor=POINT_COLOR, alpha=0.45, markersize=7, label="Runs"),
        Line2D([0], [0], color=FIT_COLOR, lw=1.8, label="Linear fit"),
    ]
    ax.legend(handles=handles, frameon=False, fontsize=9, loc="upper right")
    fig.tight_layout()
    out_path = OUT_DIR / "heterogeneous_payoff_variance_vs_min_elo_overall.png"
    fig.savefig(out_path, dpi=220, bbox_inches="tight")
    plt.close(fig)
...
def main() -> None:
    ...
    x_pad = 0.03 * (float(run_metrics["min_roster_elo"].max()) - float(run_metrics["min_roster_elo"].min()))
    x_lim = (float(run_metrics["min_roster_elo"].min()) - x_pad, float(run_metrics["min_roster_elo"].max()) + x_pad)
    y_lim = (0.0, float(run_metrics["payoff_variance"].max()) * 1.08)
    ...
    plot_specs = [
        (
            ...
            game_cols,
            "by_game",
            "Heterogeneous payoff variance vs minimum roster Elo, by game",
            "heterogeneous_payoff_variance_vs_min_elo_by_game.png",
            (11.0, 3.8),
        ),
        (
            all_rows_group,
            n_cols,
            "by_n",
            "Heterogeneous payoff variance vs minimum roster Elo, by N",
            "heterogeneous_payoff_variance_vs_min_elo_by_n.png",
            (16.0, 3.8),
        ),
        (
            all_rows_group,
            comp_cols,
            "by_competition",
            "Heterogeneous payoff variance vs minimum roster Elo, by competition band",
            "heterogeneous_payoff_variance_vs_min_elo_by_competition.png",
            (11.0, 3.8),
        ),
        (
            game_cols,
            n_cols,
            "by_game_n",
            "Heterogeneous payoff variance vs minimum roster Elo, by game and N",
            "heterogeneous_payoff_variance_vs_min_elo_by_game_n.png",
            (16.0, 8.0),
        ),
        (
            comp_cols,
            game_cols,
            "by_competition_game",
            "Heterogeneous payoff variance vs minimum roster Elo, by competition band and game",
            "heterogeneous_payoff_variance_vs_min_elo_by_competition_game.png",
            (11.0, 8.0),
        ),
    ]
    ...
    for n in N_ORDER:
        ...
        path, fits = plot_grid(
            ...
            f"Heterogeneous payoff variance vs minimum roster Elo, by game and competition band for N={n}",
            f"heterogeneous_payoff_variance_vs_min_elo_by_game_competition_n{n}.png",
            (11.0, 8.0),
            x_lim,
            y_lim,
        )
    ...
    fit_summary_path = OUT_DIR / "heterogeneous_payoff_variance_vs_min_elo_fit_summary.csv"
    ...
    print(
        "Overall fit: "
        f"n={int(overall['n_runs'])}, slope={overall['slope']:.6f}, "
        f"slope_per_100_elo={overall['slope_per_100_elo']:.3f}, "
