#!/usr/bin/env python3
"""Generate an in-place Game 2 Llama 3.2 1B backfill batch for pli-c/pli-cp."""
...
DEFAULT_OUTPUT_DIR = (
    ...
    / "experiments"
    / "results"
    / "diplomacy_20260405_082215_llama32_1b_inplace_plicp_20260411"
)
...
echo "============================================================"
echo "Diplomatic Treaty (Game 2) Llama 3.2 1B in-place pli-cp"
...
def make_sbatch_script(worker_script: Path) -> str:
    return f'''#!/bin/bash
#SBATCH --job-name=diplo-gpu1-llama32-inplace-plicp
#SBATCH --account=pli
...
#SBATCH --gres=gpu:1
#SBATCH --output=logs/cluster/diplo_gpu1_llama32_inplace_plicp_%A_%a.out
#SBATCH --error=logs/cluster/diplo_gpu1_llama32_inplace_plicp_%A_%a.err
...
echo "Submitting Llama 3.2 1B Game 2 in-place pli-cp jobs: {array_spec}%${{ONE_GPU_MAX}}"
...
def main() -> int:
    ...
    for config_id, config in selected:
        ...
        config["cluster_fallback_mode"] = "inplace_pli_cp"
