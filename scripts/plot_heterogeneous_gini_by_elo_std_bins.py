#!/usr/bin/env python3
"""Binned bar plots of heterogeneous payoff Gini by within-roster Elo std."""
...
BIN_EDGES = np.arange(0, 151, 15)
BIN_LABELS = [f"{int(left)}-{int(right)}" for left, right in zip(BIN_EDGES[:-1], BIN_EDGES[1:], strict=True)]
BAR_COLOR = "#4E79A7"
BAR_EDGE = "#2F5F8F"
...
def load_run_metrics() -> pd.DataFrame:
    ...
    run_metrics["elo_std_bin"] = pd.cut(
        run_metrics["elo_std"],
        bins=BIN_EDGES,
        labels=BIN_LABELS,
        include_lowest=True,
    ...
    if run_metrics["elo_std_bin"].isna().any():
        missing = run_metrics[run_metrics["elo_std_bin"].isna()]["elo_std"]
        raise ValueError(f"Elo std values outside bin range: {missing.min()} to {missing.max()}")
    return run_metrics
...
def summarize_bins(frame: pd.DataFrame, filters: dict[str, object] | None = None) -> pd.DataFrame:
    ...
    for idx, label in enumerate(BIN_LABELS):
        bucket = sub[sub["elo_std_bin"].astype(str).eq(label)]
        rows.append(
            {
                "bin_index": idx,
                "elo_std_bin": label,
                "elo_std_left": float(BIN_EDGES[idx]),
                "elo_std_right": float(BIN_EDGES[idx + 1]),
...
def draw_bar_axis(
    ...
    ax.set_ylim(0, y_max)
    ax.set_xticks(x)
    ax.set_xticklabels(BIN_LABELS, rotation=45, ha="right", fontsize=6.8)
...
def plot_overall(frame: pd.DataFrame) -> tuple[Path, pd.DataFrame]:
    ...
    ax.set_xlabel("Within-roster Elo standard deviation bin", fontsize=10.5)
    ax.set_ylabel("Mean corrected payoff Gini", fontsize=10.5)
    ax.set_title("Heterogeneous runs: corrected payoff Gini by Elo spread bin", fontsize=13, pad=10)
    fig.tight_layout()
    out_path = OUT_DIR / "heterogeneous_payoff_gini_by_elo_std_bin_overall.png"
    ...
    for row_idx, (row_label, _, _) in enumerate(row_groups):
        for col_idx, (col_label, _, _) in enumerate(col_groups):
            ...
            if row_idx == len(row_groups) - 1:
                ax.set_xlabel("Elo std bin", fontsize=8.5)
            else:
...
def main() -> None:
    ...
    specs = [
        (
            ...
            game_cols,
            "by_game",
            "Heterogeneous corrected payoff Gini by Elo spread bin, by game",
            "heterogeneous_payoff_gini_by_elo_std_bin_by_game.png",
            (11.0, 4.0),
        ),
        (
            all_rows,
            n_cols,
            "by_n",
            "Heterogeneous corrected payoff Gini by Elo spread bin, by N",
            "heterogeneous_payoff_gini_by_elo_std_bin_by_n.png",
            (16.0, 4.0),
        ),
        (
            all_rows,
            comp_cols,
            "by_competition",
            "Heterogeneous corrected payoff Gini by Elo spread bin, by competition band",
            "heterogeneous_payoff_gini_by_elo_std_bin_by_competition.png",
            (11.0, 4.0),
        ),
        (
            game_cols,
            n_cols,
            "by_game_n",
            "Heterogeneous corrected payoff Gini by Elo spread bin, by game and N",
            "heterogeneous_payoff_gini_by_elo_std_bin_by_game_n.png",
            (16.0, 8.2),
        ),
        (
            comp_cols,
            game_cols,
            "by_competition_game",
            "Heterogeneous corrected payoff Gini by Elo spread bin, by competition band and game",
            "heterogeneous_payoff_gini_by_elo_std_bin_by_competition_game.png",
            (11.0, 8.2),
        ),
    ]
    ...
    for n in N_ORDER:
        ...
        path, summary = plot_grid(
            ...
            f"Heterogeneous corrected payoff Gini by Elo spread bin, by game and competition band for N={n}",
            f"heterogeneous_payoff_gini_by_elo_std_bin_by_game_competition_n{n}.png",
            (11.0, 8.2),
        )
        summary["n_agents"] = n
        paths.append(path)
    ...
    run_metrics_path = OUT_DIR / "heterogeneous_payoff_gini_by_elo_std_bin_run_metrics.csv"
    summary_path = OUT_DIR / "heterogeneous_payoff_gini_by_elo_std_bin_summary.csv"
