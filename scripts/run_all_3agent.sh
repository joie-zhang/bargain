#!/bin/bash
# Simplest possible runner for 3-agent experiments - just run everything, no timeouts
set -e
...
echo "============================================================"
echo "3-AGENT EXPERIMENT RUNNER (No Timeouts)"
echo "============================================================"
...
if [ ! -f "${CONFIG_DIR}/config_0.json" ]; then
    echo "Generating 3-agent experiment configurations..."
    "${SCRIPTS_DIR}/generate_configs_3agent.sh"
fi

# Step 2: Count experiments
TOTAL_JOBS=$(ls -1 "${CONFIG_DIR}"/config_*.json 2>/dev/null | wc -l)
echo "Total 3-agent experiments: ${TOTAL_JOBS}"
echo ""
...
run_job() {
    ...
    # Run it - NO TIMEOUT
    "${SCRIPTS_DIR}/run_single_3agent_experiment.sh" ${job_id}
}
...
if [ ${FAILED} -gt 0 ]; then
    ...
    echo "  ${SCRIPTS_DIR}/run_all_3agent.sh ${MAX_PARALLEL_JOBS}"
    echo "(It will skip completed experiments automatically)"
fi

# Step 5: Collect results
echo ""
echo "Collecting 3-agent results..."
"${SCRIPTS_DIR}/collect_3agent_results.sh"
