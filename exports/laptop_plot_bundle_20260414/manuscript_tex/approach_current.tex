In \Cref{sec:game_1_problem_setting}, \Cref{sec:game_2_problem_setting}, and \Cref{sec:game_3_problem_setting}, we outline the mathematical formalism for our three negotiation frameworks. In \Cref{sec:game_1_experimental_setup}, \Cref{sec:game_1_multiagent_setup}, \Cref{sec:game_2_experimental_setup}, and \Cref{sec:game_3_experimental_setup}, we describe the key design choices in our experimental setups.
...
Intuitively, $\alpha_{ij} = 1$ suggests the highest level of competition because agents have the same preferences across all items, and each item can only be allocated to one agent; agent $i$'s gain is agent $j$'s loss.
...
We evaluate competition levels in $\{0.0, 0.25, 0.5, 0.75, 0.9, 0.95, 1.0\}$, discussion-turn settings in $\{1, 2\}$, and both model orders (\texttt{weak\_first}, \texttt{strong\_first}). This yields 28 games per model pair in the base grid.
We fix GPT-5-nano as the baseline and compare against the active April 2026 adversary roster used in the analysis scripts.
As capability proxies, we use parameter count for open-source models when available and LMArena Elo \citep{chiang2024chatbotarenaopenplatform} (January 16, 2026 snapshot).
...
\begin{enumerate}
    ...
    \item \textbf{Discussion:} Agents take turns discussing their preferences. Agents may strategically misreport preferences.
...
Unlike Game 1, in which competition is controlled by a single parameter and outcomes are discrete, the treaty game has continuous issue values.
Each agent's per-issue value function $1 - |p^i_k - a_k|$ is single-peaked at $a_k = p^i_k$, so agents can make partial concessions.
...
\end{enumerate}
The latent correlation $\rho_z$ is calibrated so that the Pearson correlation\footnote{The Pearson correlation coefficient measures the linear association between two random variables, ranging from $-1$ (perfect negative) to $+1$ (perfect positive).} of sampled positions equals $\rho_{\text{target}}$, using $\rho_z = 2\sin(\pi\,\rho_{\text{target}} / 6)$.
...
Weight vectors are generated with SLSQP optimization under simplex constraints to match a target overlap.
...
The key difference is in the initialization stage: each agent receives both a position preference vector and an importance weight vector over continuous issues, and is instructed to propose agreements in $[0,1]^m$.
...
For agents $i$ and $j$, alignment is measured as cosine similarity of valuation vectors, the same measure used in Game 1:
\begin{equation}
...
These two parameters are orthogonal: $\alpha$ controls preference alignment, while $\sigma$ controls resource scarcity.
For example, $(\alpha, \sigma) = (1, 0.2)$ is a high-alignment but scarce-budget setting, while $(\alpha, \sigma) = (0, 1)$ is a low-alignment but high-budget setting.

In the current implementation, Game 3 uses a propose-and-vote loop.
At each round, each agent submits a contribution vector over projects.
The environment aggregates all submitted vectors into one joint proposal and computes the funded set under threshold constraints.
Agents then vote privately to accept or reject this joint proposal.
If all agents accept in round $t$, that proposal is locked in and realized utilities are discounted by $\gamma^{t-1}$.
If the vote is not unanimous, the game advances to the next round.
If no joint proposal is unanimously accepted by round $T$, all agents receive utility 0.
...
In Game 3, the simplex constraint on valuations ($\sum_j v^i_j = 100$) guarantees that the \emph{theoretical} maximum utility is 100 (all projects funded and the agent pays nothing, i.e., pure free-riding).
...
This is by design: variation in achievable utility across $\sigma$ levels is the signal of interest, since it captures the cost of scarcity.
...
The need for cooperative solution concepts arises because the Nash equilibria of the extensive-form negotiation games (Games 1 and 2) are massively multiple, including degenerate equilibria where all agents reject all proposals and receive utility 0.
...
The core analysis for Game 3 supplements this decomposition by identifying whether unfairness is severe enough that a subset of agents would have been better off breaking away from the grand coalition entirely, which is a stronger stability criterion than the Lindahl benchmark alone.
