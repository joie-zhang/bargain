...
Unlike Games 1 and 2, which use a propose-and-vote protocol where agents collectively select a joint plan, the co-funding game uses a \emph{decentralized contribution protocol} in which each agent submits \emph{individual} contribution vectors and the threshold logic mechanically determines the outcome.
This design follows the standard \emph{provision point mechanism} (PPM) with money-back guarantee from the threshold public goods literature (Bagnoli \& Lipman, 1989), and preserves the coordination problem that makes co-funding strategically interesting.
...
\begin{enumerate}
    ...
    \item \textbf{Contribution (Pledge):} Each agent $i$ submits a contribution vector $\bm{x}^i_t = (x^i_{t,1}, \ldots, x^i_{t,M})$ subject to $\sum_j x^i_{t,j} \leq B_i$.
...
%% [CHANGED: promoted from §7.3 design note to main protocol]
If all agents submit identical contribution vectors for 2 consecutive rounds ($\bm{x}^i_t = \bm{x}^i_{t-1}$ for all $i$), the game terminates early and the current contributions determine the outcome.
