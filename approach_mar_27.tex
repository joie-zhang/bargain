...
Given two valuation vectors $\bm{v}^i$ and $\bm{v}^j$, we measure the degree of competition between agents $i$ and $j$ as the \emph{cosine similarity} $\alpha_{ij} = \frac{{\bm{v}^i} \cdot \bm{v}^j}{\norm{\bm{v}^i} \norm{\bm{v}^j}} \in [0,1]$.
Intuitively, $\alpha_{ij} = 1$ suggests the highest level of competition because the agents have the same preferences across all items, and yet each item can only be allocated to one agent -- thus, agent $i$'s gain is agent $j$'s loss.
Similarly, when $\alpha_{ij} = 0$, the agents' valuations are completely orthogonal, meaning it is possible to allocate all items and maximize both agents' utilities while doing so (the most cooperative outcome).

For $n > 2$ agents, we measure the overall degree of competition as the average pairwise cosine similarity:
\begin{equation}
    \bar{\alpha} = \frac{2}{n(n-1)} \sum_{i < j} \alpha_{ij}.
\end{equation}
This can be interpreted as a measure of \emph{collective misalignment}: when $\bar{\alpha}$ is high, agents' preferences point in similar directions on the simplex, so the rivalrous nature of the items creates strong conflict; when $\bar{\alpha}$ is low, preferences are spread out, enabling mutually beneficial allocations.
...
The first is a \emph{position preference} vector $\bm{p}^{i} = (p^{i}_{1}, \dots, p^{i}_{m})$ where $p^{i}_{k} \in [0,1]$, which specifies agent $i$'s ideal outcome on each issue $k$.
The second is an \emph{importance weight} vector $\bm{w}^{i} = (w^{i}_{1}, \dots, w^{i}_{m})$ where $w^{i}_{k} \geq 0$ and $\sum_{k} w^{i}_{k} = 1$, which specifies how much agent $i$ cares about each issue relative to the others.
...
The factor of 100 normalizes the utility range to $[0, 100]$, consistent with Games 1 and 3.

Unlike Game 1, in which competition is controlled by a single parameter (the cosine similarity of valuations) and items are discrete, the treaty game features \emph{continuous} outcomes on each issue.
Moreover, each agent's per-issue value function $1 - |p^i_k - a_k|$ is \emph{single-peaked}: it achieves a unique maximum at $a_k = p^i_k$ and decreases symmetrically on either side.
This single-peaked structure---familiar from the spatial voting literature \citep{black1948rationale}---makes the treaty game qualitatively richer than item allocation: agents can make partial concessions, and the distance between ideal positions on each issue determines the room for compromise.

The treaty negotiation game exposes \emph{two} independent axes of competition.
...
Position preferences are generated using a Gaussian copula\footnote{A Gaussian copula is a method for generating correlated random variables with specified marginal distributions. It works by first drawing correlated normal variables, then applying the normal CDF $\Phi$ to transform each into a uniform variable, preserving the correlation structure while ensuring uniform marginals.} that yields exact $\text{Uniform}(0,1)$ marginals with controlled Pearson correlation:
\begin{enumerate}
...
\end{enumerate}
The latent correlation $\rho_z$ is calibrated so that the Pearson correlation\footnote{The Pearson correlation coefficient measures the linear association between two random variables, ranging from $-1$ (perfect negative) to $+1$ (perfect positive).} of the resulting positions equals $\rho_{\text{target}}$, via the closed-form relationship $\rho_z = 2\sin(\pi\,\rho_{\text{target}} / 6)$.
For $n > 2$ agents with equicorrelation, positive semi-definiteness\footnote{A matrix is positive semi-definite (PSD) if all its eigenvalues are non-negative. Correlation matrices must be PSD; this constrains how negatively correlated multiple variables can simultaneously be.} of $\Sigma_z$ requires $\rho_z \geq -1/(n-1)$, which restricts the feasible negative range (e.g., $\rho \geq -0.25$ for $n = 5$).
...
Weight vectors are generated with exact target overlap using SLSQP (Sequential Least-Squares Quadratic Programming) optimization: $\min_{\bm{w}^1, \bm{w}^2} (\cos(\bm{w}^1, \bm{w}^2) - \theta_{\text{target}})^2$ subject to $w^i_k \geq 0$ and $\sum_k w^i_k = 1$.
For $n > 2$ agents, we jointly optimize all $n$ weight vectors to match the target pairwise cosine similarity: $\min_{\bm{w}^1, \ldots, \bm{w}^n} \sum_{i < j} (\cos(\bm{w}^i, \bm{w}^j) - \theta_{\text{target}})^2$, subject to the same simplex constraints.
This achieves target cosine similarities up to numerical tolerance, and is identical to the parameter generation used for preference alignment in Games 1 and 3.
...
Note that this setting is ``overdetermined'' in the sense that either condition alone ($\rho = 1$ or $\theta = 0$) would substantially reduce conflict; having both simultaneously makes cooperation trivial.
...
Note that $\rho = -1$ is infeasible for $m > 2$: for uniformly distributed vectors on $[0,1]^m$, the minimum achievable Pearson correlation converges to approximately $-1/(m-1)$, giving $\rho_{\min} \approx -0.25$ for $m = 5$.
The Gaussian copula correctly enforces this constraint: if the target $\rho$ requires a latent correlation that violates positive semi-definiteness, the Cholesky decomposition of $\Sigma_z$ fails, raising an error rather than silently producing biased results.
...
As a scalar summary of overall misalignment, one can integrate the expected utility difference across agreements:
$\mathcal{D}(\rho, \theta) = \mathbb{E}_{\bm{a} \sim \text{Uniform}([0,1]^m)} \left[\text{Var}_i\{u^i(\bm{a})\}\right]$,
which measures how much agents disagree, on average, about the value of a randomly chosen agreement.
This quantity increases with both $|\rho|$ (when preferences point in opposite directions) and $\theta$ (when agents contest the same issues), and could be visualized as a heatmap over the $(\rho, \theta)$ grid.
...
In our implementation of the problem setting above, we set $n = 2$, $m = 5$, $T = 10$, and $\gamma = 0.9$.
We generate preference instances across a grid of competition levels: for $\rho$, we use levels spanning the feasible range (approximately $[-0.25, 1]$ for $m = 5$; see above), and for $\theta$, we use 11 levels from $0$ to $1$ in increments of $0.1$.
...
The key difference is in the initialization stage: rather than receiving a single valuation vector over discrete items, each agent receives both a position preference vector and an importance weight vector over continuous issues, and is instructed to propose agreements as vectors of values in $[0,1]^m$.
...
Unlike Games 1 and 2, in which utility is a linear function of the outcome, the co-funding game introduces a \emph{threshold non-linearity}: each project provides zero value until its cost is met, at which point all agents receive their full value from the project regardless of how much they individually contributed.
This threshold structure places the game within the class of \emph{threshold public goods games} \citep{palfrey1984testing}, in which a collective benefit is realized only once contributions exceed a critical mass.
...
For agents $i$ and $j$, alignment is measured as the cosine similarity of their valuation vectors---the same measure used in Game 1:
\begin{equation}
    \alpha_{ij} = \frac{\bm{v}^{i} \cdot \bm{v}^{j}}{\norm{\bm{v}^{i}} \norm{\bm{v}^{j}}} \in [0,1].
\end{equation}
Note that although $\alpha_{ij}$ is computed identically in Games 1 and 3, its \emph{interpretation} differs: in Game 1 (rivalrous goods), high $\alpha$ means high competition because agents want the same scarce items; in Game 3 (public goods with co-funding), high $\alpha$ means high cooperation potential because agents share the benefits of funded projects.
...
For $n > 2$ agents, the overall alignment is $\bar{\alpha} = \frac{2}{n(n-1)} \sum_{i < j} \alpha_{ij}$.
...
For example, $(\alpha, \sigma) = (1, 0.3)$ yields a pure cooperative prioritization problem: agents agree on everything but must jointly choose which projects to fund with scarce budgets (here $\sigma = 0.3$ means the total budget is 30\% of total project costs, one of our experimental grid values; see \Cref{sec:game_3_experimental_setup}).
Conversely, $(\alpha, \sigma) = (0, 0.3)$ yields pure competition: agents want entirely different projects and fight over a limited shared budget.
...
Instead, each agent submits \emph{individual contribution vectors}---pledges of how to allocate their own budget across projects---and the threshold logic determines which projects are funded.
This design choice follows the \emph{provision point mechanism} (PPM) from the threshold public goods literature \citep{bagnoli1989provision}, in which agents make decentralized contribution decisions rather than voting on joint allocation plans.
The PPM preserves the coordination problem that makes co-funding strategically interesting: agents must independently decide how much to pledge toward each project without certainty about others' contributions, creating genuine risks of free-riding and coordination failure that would be eliminated by a centralized propose-and-vote mechanism.

The rationale for using a decentralized contribution protocol rather than a propose-and-vote scheme (as in Games 1 and 2) is that it preserves the \emph{threshold coordination problem}---the strategically interesting aspect of co-funding.
Under propose-and-vote, agents would simply vote on complete funding plans, and the threshold non-linearity would become irrelevant: any feasible funded set could be proposed directly, collapsing Game 3 into something resembling the treaty negotiation.
By contrast, the decentralized protocol forces agents to independently commit resources without certainty about others' contributions, which is precisely the strategic tension that distinguishes co-funding from the other games.
The absence of a discount factor follows naturally: since there is no ``agreement'' event (only evolving contribution vectors), there is no natural round to discount from.
...
Project costs are drawn uniformly at random from $[10, 30]$, and agent budgets are set to $b^i = \sigma \cdot \sum_j c_j / n$ so that budgets are equal across agents.
...
\begin{enumerate}
    ...
    \item \textbf{Contribution (Pledge):} Each agent $i$ submits a contribution vector $\bm{x}^i_t = (x^i_{t,1}, \ldots, x^i_{t,m})$ subject to $\sum_j x^i_{t,j} \leq b^i$. These are \emph{revisable pledges}: agents may increase, decrease, or reallocate their contributions relative to previous rounds.
...
If all agents submit identical contribution vectors for two consecutive rounds ($\bm{x}^i_t = \bm{x}^i_{t-1}$ for all $i$), the game terminates early and the current contributions determine the outcome.
...
\begin{itemize}
    ...
    \item \textbf{Adaptation rate}: $\text{Adapt}_i = \frac{1}{T-1} \sum_{t=2}^{T} \|\bm{x}^i_t - \bm{x}^i_{t-1}\|_1 / b^i$, computable purely from contribution vectors without transcript analysis.
    \item \textbf{Persuasion effectiveness}: Whether agent $i$ \emph{advocating for} project $j$ in communication at round $t$ predicts an increase in other agents' contributions to $j$ at round $t+1$. We distinguish positive advocacy (``we should fund project $j$'') from negative advocacy (``don't fund project $j$''), as the two should have opposite predicted effects. This is a Granger-causality-style measure linking communication to subsequent behavior shifts.
...
In Game 2, we normalize utility to $[0, 100]$ via the multiplicative factor of 100 in the utility function.
...
Together, these guarantee $u^i \in [0, 100]$ for all agents and all configurations: the maximum of 100 is achieved when the agreement matches every agent's ideal position ($a_k = p^i_k$ for all $k$), and the minimum of 0 is achieved at timeout.
Crucially, the position vectors $\bm{p}^i$ are \emph{not} on a simplex (they lie in $[0,1]^m$ with no sum constraint), and this is correct: positions represent independent ideal outcomes on separate issues, and forcing a tradeoff between them would be semantically inappropriate.
...
For all three games, the socially optimal outcome can be computed exactly and efficiently at our experimental scale ($n = 2$, $m = 5$).
...
For Game 2 ($n=2$, $m=5$), the NBS maximizes $u^1(\bm{a}) \cdot u^2(\bm{a})$ over $\bm{a} \in [0,1]^m$; since the objective is smooth, this is solved via L-BFGS-B (Limited-memory Broyden--Fletcher--Goldfarb--Shanno with Box constraints), a standard quasi-Newton optimization method for box-constrained problems.
...
At the Lindahl equilibrium, $F_{ij} = 1$ for every agent on every project---nobody overpays or underpays relative to their benefit.
...
The core analysis for Game 3 supplements this decomposition by identifying whether the unfairness is severe enough that a subset of agents would have been better off breaking away from the grand coalition entirely---a stronger stability criterion than the Lindahl benchmark alone.
